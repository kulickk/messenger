...
export async function subscribeNotifications() {
    try {
        ...
        console.log(
            "[SW] Received push subscription: ",
            JSON.stringify(pushSubscription),
        );

        const { endpoint } = pushSubscription;
        const p256dh_base64url = arrayBufferToBase64(
            pushSubscription.getKey("p256dh"),
        );
        const auth_base64url = arrayBufferToBase64(
        ...
        console.error(
            "[SW] Error during service worker push subscription: ",
            error,
        );
